...
const routeTimeNowButton = document.getElementById("routeTimeNowButton");
...
function isRouteTimeChangeAllowed() {
  return state === "standby" || state === "home";
}

function updateRouteTimePanelState() {
  routeTimePanel.classList.toggle("manual", routeTimeManual);
  routeTimePanel.title = routeTimeManual
    ? `検索時刻 ${routeSearchTime}（手動指定中）`
    : `検索時刻 ${routeSearchTime}（現在時刻）`;
}
...
function openRouteTimePicker() {
  if (!isRouteTimeChangeAllowed()) {
    toast("この画面では検索時刻を変更できません");
    return;
  }

  routeTimeInput.value = routeSearchTime;
  if (typeof routeTimeInput.showPicker === "function") {
    routeTimeInput.showPicker();
    return;
  }

  const value = window.prompt("検索時刻を入力してください（例 07:42）", routeSearchTime);
  if (value === null) return;
  setRouteSearchTime(value, { manual: true });
}
function resetRouteSearchTimeToNow() {
  if (!isRouteTimeChangeAllowed()) {
    toast("この画面では検索時刻を変更できません");
    return;
  }

  setRouteSearchTime(currentTime, { manual: false });
}
...
routeTimeNowButton.addEventListener("click", resetRouteSearchTimeToNow);
